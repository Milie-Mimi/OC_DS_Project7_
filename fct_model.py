...
def DummyClassifier_model(xtrain, ytrain, xtest, ytest):
...
def RegLog_model(xtrain, ytrain, xtest, ytest):
    """Fonction qui utilise un pipeline de transformation des données et applique
...
def lgb_model(xtrain, ytrain, xtest, ytest):
    """Fonction qui utilise un pipeline de transformation des données et applique
...
def lgb_feat_imp_5(xtrain, ytrain, xtest, ytest):
    """Fonction qui utilise un pipeline de transformation des données et applique
    un LightGBM pour effectuer la prédiction. Le pipeline est optimisé et entrainé en fonction
    du scoring f betascore (avec beta = 9), cross validation et d'une grille de paramètres.
    ...
    with mlflow.start_run(experiment_id=experiment.experiment_id, run_name='LightGBM - Feature Importance *5'):
        ...
        lgb5, lgb_params5, lgb_name5, lgb_duration5 = best_model(model_name='LightGBM - Feature Importance *5',
                                                                 ...
                                                                 cv=KFold(n_splits=5,
                                                                          shuffle=True,
                                                                          random_state=42),
                                                                 xtrain=xtrain[feat_lgb5],
                                                                 ytrain=ytrain,
                                                                 preprocessor=StandardScaler(),
                                                                 params=params,
                                                                 scoring=make_scorer(fbeta_score, beta=9),
                                                                 xtest=xtest[feat_lgb5],
                                                                 ytest=ytest,
                                                                 oversampling_strategy=0.1,
                                                                 undersampling_strategy=0.5,
                                                                 balanced=False,
...
def lgb_feat_imp_10(xtrain, ytrain, xtest, ytest):
    """Fonction qui utilise un pipeline de transformation des données et applique
    un LightGBM pour effectuer la prédiction. Le pipeline est optimisé et entrainé en fonction
    du scoring f betascore (avec beta = 9), cross validation et d'une grille de paramètres.
    ...
    with mlflow.start_run(experiment_id=experiment.experiment_id, run_name='LightGBM - Feature Importance *10'):
        ...
        lgb10, lgb_params10, lgb_name10, lgb_duration10 = best_model(
            ...
            cv=KFold(n_splits=5,
                     shuffle=True,
                     random_state=42),
            xtrain=xtrain[feat_lgb10],
            ytrain=ytrain,
            preprocessor=StandardScaler(),
            params=params,
            scoring=make_scorer(fbeta_score, beta=9),
            xtest=xtest[feat_lgb10],
            ytest=ytest,
            oversampling_strategy=0.1,
            undersampling_strategy=0.5,
            balanced=False,
...
def lgb_feat_imp_20(xtrain, ytrain, xtest, ytest):
    """Fonction qui utilise un pipeline de transformation des données et applique
    un LightGBM pour effectuer la prédiction. Le pipeline est optimisé et entrainé en fonction
    du scoring f betascore (avec beta = 9), cross validation et d'une grille de paramètres.
    ...
    with mlflow.start_run(experiment_id=experiment.experiment_id, run_name='LightGBM - Feature Importance *20'):
        ...
        lgb20, lgb_params20, lgb_name20, lgb_duration20 = best_model(
            ...
            cv=KFold(n_splits=5,
                     shuffle=True,
                     random_state=42),
            xtrain=xtrain[feat_lgb20],
            ytrain=ytrain,
            preprocessor=StandardScaler(),
            params=params,
            scoring=make_scorer(fbeta_score, beta=9),
            xtest=xtest[feat_lgb20],
            ytest=ytest,
            oversampling_strategy=0.1,
            undersampling_strategy=0.5,
            balanced=False,
...
def lgb_feat_imp_30(xtrain, ytrain, xtest, ytest):
    """Fonction qui utilise un pipeline de transformation des données et applique
    un LightGBM pour effectuer la prédiction. Le pipeline est optimisé et entrainé en fonction
    du scoring f betascore (avec beta = 9), cross validation et d'une grille de paramètres.
    ...
    with mlflow.start_run(experiment_id=experiment.experiment_id, run_name='LightGBM - Feature Importance *30'):
        ...
        lgb30, lgb_params30, lgb_name30, lgb_duration30 = best_model(
            ...
            cv=KFold(n_splits=5,
                     shuffle=True,
                     random_state=42),
            xtrain=xtrain[feat_lgb30],
            ytrain=ytrain,
            preprocessor=StandardScaler(),
            params=params,
            scoring=make_scorer(fbeta_score, beta=9),
            xtest=xtest[feat_lgb30],
            ytest=ytest,
            oversampling_strategy=0.1,
            undersampling_strategy=0.5,
            balanced=False,
...
def lgb_gscv(xtrain, ytrain, xtest, ytest):
    """Fonction qui utilise un pipeline de transformation des données et applique
    un LightGBM pour effectuer la prédiction. Le pipeline est optimisé et entrainé en fonction
    du scoring f betascore (avec beta = 9), cross validation et d'une grille de paramètres.
...
def lgb_beta2(xtrain, ytrain, xtest, ytest):
    """Fonction qui utilise un pipeline de transformation des données et applique
    un LightGBM pour effectuer la prédiction. Le pipeline est optimisé et entrainé en fonction
...
def lgb_flaml(xtrain, ytrain, xtest, ytest):
    """Fonction qui utilise un pipeline de transformation des données et applique
    un LightGBM pour effectuer la prédiction. Le pipeline est optimisé et entrainé en fonction
    du scoring f betascore (avec beta = 9), cross validation et d'une grille de paramètres.
    ...
    with mlflow.start_run(experiment_id=experiment.experiment_id, run_name='LightGBM 30 - FLAML'):
        ...
        lgb30_FLAM.fit(xtrain[feat_lgb30], ytrain)
...
def lgb_flaml_no_pipeline(xtrain, ytrain, xtest, ytest):
    """Fonction qui utilise un pipeline de transformation des données et applique
        un LightGBM pour effectuer la prédiction. Le pipeline est optimisé et entrainé en fonction
        du scoring f betascore (avec beta = 9), cross validation et d'une grille de paramètres.
    ...
    X_test_scaled = scaler.transform(xtest[feat_lgb30])
    ...
    with mlflow.start_run(experiment_id=experiment.experiment_id, run_name='Lgb - no pipeline'):
        ...
        credit_score_model_LIME = LGBMClassifier(n_estimators=31204,
                                                 ...
                                                 random_state=42,
                                                 class_weight='balanced')
        credit_score_model_LIME.fit(X_train_scaled, ytrain)
        ...
        (biz_lgb_no_pipe, beta_lgb_no_pipe, rec_lgb_no_pipe, prec_lgb_no_pipe,
         acc_lgb_no_pipe, auc_lgb_no_pipe, y_pred_lgb_no_pipe) = eval_metrics(best_model=credit_score_model_LIME,
                                                                              xtest=X_test_scaled,
                                                                              ytest=ytest,
                                                                              beta_value=9)
        ...
        mlflow.sklearn.log_model(credit_score_model_LIME, "credit_score_model_LIME")

        return credit_score_model_LIME


# def LIME_explainer(xtrain, ytrain, xtest, ytest):
...
def data_app(nrows):
    ...
    # Définition des features et de la target
    feat_lgb30 = ['SK_ID_CURR', 'CREDIT_DURATION', 'EXT_SOURCE_2', 'INSTAL_DAYS_PAST_DUE_MEAN',
                  'PAYMENT_RATE', 'POS_CNT_INSTALMENT_FUTURE_MEAN', 'CREDIT_GOODS_PERC',
                  'AGE', 'POS_NB_CREDIT', 'BURO_CREDIT_ACTIVE_Active_SUM', 'BURO_AMT_CREDIT_SUM_DEBT_MEAN',
                  'YEARS_EMPLOYED', 'YEARS_ID_PUBLISH', 'INSTAL_PAYMENT_DIFF_MEAN', 'BURO_AMT_CREDIT_SUM_MEAN',
                  'AMT_ANNUITY', 'AMT_GOODS_PRICE', 'BURO_YEARS_CREDIT_ENDDATE_MEAN', 'AMT_CREDIT',
    ...
    df, categ_feat = eda.categories_encoder(df, nan_as_category=False)
...
if __name__ == '__main__':
    ...
    lgb_no_pipe = lgb_flaml_no_pipeline(xtrain=X_train,
                                        ytrain=y_train,
                                        xtest=X_test,
                                        ytest=y_test)
